...
r2.filter_asm = function (x, display) {
  let curoff = backward ? prev_curoff : next_curoff;

  let lastoff = backward ? prev_lastoff : next_lastoff;
  ...
  for (var i = lines.length - 1; i > 0; i--) {
    const a = lines[i].match(/0x([a-fA-F0-9]+)/);
    if (a && a.length > 0) {
